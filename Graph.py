...
def _fig_size(feature_names, top_idx, base_w=8, base_h=9, h_per_row=0.6):
    #Calculate figure size accounting for long feature names.
    max_chars = max(len(feature_names[i]) for i in top_idx)
    extra_w = max(0, (max_chars - 20) * 0.08)  # add width for names > 20 chars
    h = max(base_h, len(top_idx) * h_per_row + 1.5)
    return (base_w + extra_w, h)


def plot_bar(sv, feature_names, top_idx, target, model_name):
    mean_abs_shap = np.abs(sv).mean(axis=0)

    fig, ax = plt.subplots(figsize=_fig_size(feature_names, top_idx, base_w=8, base_h=9))
    ax.barh(feature_names[top_idx], mean_abs_shap[top_idx], color='steelblue')
...
def plot_violin(sv, X_test_array, feature_names, top_idx, target, model_name):
    fig, ax = plt.subplots(figsize=_fig_size(feature_names, top_idx, base_w=9, base_h=7))
